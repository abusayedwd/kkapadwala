...
const transport = nodemailer.createTransport(config.email.smtp as nodemailer.TransportOptions);
if (config.env !== 'test') {
  transport
    .verify()
...
const sendEmailVerification = async (to: string, otp: number | string) => {
...
const sendResetPasswordEmail = async (to: string, otp: number | string) => {
